...
local function test(name, aliases, callback, target)
	...
	task.spawn(function()
		...
		elseif not callback then
			...
			elseif message == "attempt to call a nil value" then
				fails = fails + 1
				warn("⛔ " .. name)
			else
				fails = fails + 1
...
print("✅ - Pass, ☑️ - Only global declared, ⚠️ - Missing aliases, ⛔ - Fail\n")

task.defer(function()
	repeat task.wait() until running == 0
	if rconsoledestroy ~= nil then rconsoledestroy() end
...
test("decompile", {}, function()
	local animate = game:GetService("Players").LocalPlayer.Character.Animate
	local dec = decompile(animate)
	assert(typeof(dec)=="string", "Should return string")
end)
